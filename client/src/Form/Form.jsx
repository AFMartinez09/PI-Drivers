...
function form() {
  ...
  const handleChange = (event) => {
    ...
    setErrors(validateField({ name, value }, errors));
  }
  ...
  return (
    <form className={style.form} onSubmit={handleSubmit}>
      ...
      <input autoComplete='off' className={style.surnameInput} type="text" name="surname" value={driverData.surname} onChange={handleChange} placeholder="Enter driver's lastname" />
      {errors.surname && <p className={style.surnameError}>{errors.surname}</p>}

      <label className={style.image} html For="image">Image</label>
      <input autoComplete='off' className={style.imageInput} type="file" accept="image/*" name="image" onChange={handleChange} placeholder="Enter the link image" />
      ...
      <button className={style.submit} type="submit" disabled={validation(errors)}>Submit</button>
    </form>
  )
}
